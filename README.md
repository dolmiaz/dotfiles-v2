...
導入後、npm のグローバル prefix を `~/.local` に設定し（root 不要のグローバルインストール）、npmrc は XDG 準拠の `~/.config/npm/npmrc` を使います。
...
- 各コンポーネント（base パッケージ、CLI ツール、C/C++、Rust、uv、npm prefix、VS Code、zsh プラグイン）の健全性
...
|---|---|
| `--fix` | sudo 不要な問題を自動修復 |
| `--fix-sudo` | sudo が必要な問題も含めてすべて自動修復 |
| `--quiet` | 問題のある項目だけを表示 |
| `-h`, `--help` | ヘルプを表示 |
...
- `install.sh` は**再実行しても安全**です。内容が変わっていないファイルはスキップされ、変更のあるファイルだけが更新されます。
