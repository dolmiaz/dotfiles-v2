...
- settings.json の配置先: macOS は `~/Library/Application Support/Code/User/`、Linux は `~/.config/Code/User/`
...
- credential helper は OS に応じて自動選択（macOS: `osxkeychain`、Linux: `git-credential-libsecret` があれば `libsecret`、なければ `cache --timeout=86400`）
- merge conflict style は Git 2.35 以上なら `zdiff3`、それ未満なら互換性のため `diff3`
...
```sh
./doctor.sh          # 診断のみ
./doctor.sh --fix    # sudo 不要な問題のみ修復を試みる
./doctor.sh --fix-sudo # パッケージ導入など sudo が必要な修復も実行する
```
...
- 配布元に存在しないパッケージ（例: Ubuntu 22.04 の `eza`）は CLI ツールの FAIL 対象外です
- git の user.name / user.email 設定
- デフォルトシェルが zsh か（修復には `--fix-sudo` が必要）
- `/etc/sudoers.d/dotfiles-path` は任意項目（存在すれば OK / なければ SKIP。installer は配布しないため、必要な場合は手動設定）
...
| オプション | 説明 |
|---|---|
| `--fix` | sudo 不要な問題だけ自動修復を試みる（自動修復手段のない項目は FAIL のまま報告） |
| `--fix-sudo` | パッケージ導入系やデフォルトシェル変更、npm ディレクトリ所有権など sudo が必要な項目も修復対象に含める |
| `--quiet` | 問題のある項目だけを表示 |
| `-h`, `--help` | ヘルプを表示 |
...
- `--link` でも `~/.zshrc`（landing pad）は外部ツールが追記するため、常に copy で配置します。
- `--link` でも `~/.config/npm/npmrc` は npm が machine-local な値を書き込むため、常に copy で配置します。
...
- `doctor.sh --fix` は sudo 不要の修復のみを行います。パッケージ導入系の修復は `--fix-sudo` を使ってください。PATH 設定、git ユーザー設定など自動修復手段のない項目は手動対応が必要です。
